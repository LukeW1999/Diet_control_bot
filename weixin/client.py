...
def send_text(token: str, to_user_id: str, context_token: str, text: str) -> dict:
    ...
    """
    # Shape taken from @tencent-weixin/openclaw-weixin 2.4.8 (messaging/send.js).
    # Without `client_id` the server still answers with a message_id and the
    # message never arrives, so a missing one looks exactly like success.
    msg = {
        "from_user_id": "",
        "to_user_id": to_user_id,
        "client_id": f"health-bot:{int(time.time() * 1000)}-{os.urandom(4).hex()}",
        "message_type": 2,   # BOT
