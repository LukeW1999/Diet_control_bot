...
def save_note(entry_date: date, category: str, content: str, summary: str = "") -> Path:
    ...
    filepath = _notes_dir() / f"{entry_date.isoformat()}.md"
...
def get_week_notes(start: date, end: date) -> str:
    ...
    current = start
    while current <= end:
        filepath = _notes_dir() / f"{current.isoformat()}.md"
        if filepath.exists():
...
def get_today_notes(target_date: date) -> str:
    filepath = _notes_dir() / f"{target_date.isoformat()}.md"
